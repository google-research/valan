...
"""3/4-Layer lingunet with input and output spatial dimensions at 1x8 and 1x18.

The intended use-case for this LingUNet implementation is to map a 1x8
panoramic StreetView feature map to a 1x18 output.

This specific module uses three convolutions at the encoding stage and four
deconvolutions for decoding in order to increase the output spatial resolution.
"""
...
class LingUNet3(tf.keras.Model):
  ...
  def __call__(self, input_image, text_embed, training=False):
    ...
    assert_shape((batch_size, 1, 8, self.in_channels), input_image.shape)
    ...
    x1 = utils.circular_pad(input_image, axis=2, padding=[1, 2])
    assert_shape((batch_size, 1, 11, self.in_channels), x1.shape)
    ...
    #      /|\ /|\ /|\    <`
    #     X X X X X X X
    #     - | | | | | -   <- Cut off the marging that doesn't correspond to
    ...
    d0 = self.deconv1(d1)
    out = d0[:, :, 0:-1, :]  # Crop off the right margin
